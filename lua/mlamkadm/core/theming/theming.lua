...
M.themes = {
    ...
    tokyonight = {
        ...
        setup = function()
            ...
            if not ok then
                vim.notify(
                    "tokyonight.nvim plugin not found. Please make sure it's installed via your plugin manager.",
                    vim.log.levels.WARN,
                    { title = "Theme Manager" }
                )
                return false
    ...
    dracula = {
        ...
        setup = function()
            ...
            if not ok then
                vim.notify(
                    "dracula.nvim plugin not found. Please make sure it's installed via your plugin manager.",
                    vim.log.levels.WARN,
                    { title = "Theme Manager" }
                )
                return false
    ...
    onedark = {
        ...
        setup = function()
            ...
            if not ok then
                vim.notify(
                    "onedark.nvim plugin not found. Please make sure it's installed via your plugin manager.",
                    vim.log.levels.WARN,
                    { title = "Theme Manager" }
                )
                return false
...
function M.switch_theme(theme_name)
    ...
    if theme_name ~= "gruvbox" then -- gruvbox is already loaded via config
        ...
        if not plugin_loaded then
            vim.notify(
                "Could not load " .. theme_info.plugin .. ". Attempting to apply theme anyway.",
                vim.log.levels.WARN,
                { title = "Theme Manager" }
            )
        end
    ...
    if not success then
        vim.notify(
            "Error setting up theme " .. theme_name .. ": " .. tostring(err),
    ...
    if saved_theme and M.themes[saved_theme] then
        ...
        if saved_theme == "gruvbox" then
            ...
            if ok then
                ...
                vim.notify(
                    "Failed to restore " .. theme_info.name .. " theme: " .. tostring(err),
                    vim.log.levels.ERROR,
                    { title = "Theme Manager" }
                )
            end
